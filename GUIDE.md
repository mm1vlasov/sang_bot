...
- **Формат:** массив ID ролей, например:
  ```json
  "commands": ["1382738163729956947", "1382738220797788322"]
  ```
...
- **Пример:** только «Старший состав»:
  ```json
  "commands": ["1382738163729956947"]
  ```
- **Пример:** добавить третью роль:
  ```json
  "commands": ["1382738163729956947", "1382738220797788322", "ЕЩЕ_ID_РОЛИ"]
  ```
...
- **Формат:** массив ID ролей, например:
  ```json
  "resignPromotionSubmit": ["1382738255249936554"]
  ```
...
- **Пример:** разрешить двум ролям:
  ```json
  "resignPromotionSubmit": ["1382738255249936554", "ID_ВТОРОЙ_РОЛИ"]
  ```
...
- **Формат:** массив ID ролей, например:
  ```json
  "resignPromotionApprove": ["1382738163729956947"]
  ```
...
- **Пример:** две роли могут одобрять:
  ```json
  "resignPromotionApprove": ["1382738163729956947", "1382738220797788322"]
  ```
...
- **Формат:** массив ID ролей:
  ```json
  "inviteRoles": ["1382738255249936554", "1450198860419960935"]
  ```
...
- **Как поменять:**
  ...
  - Чтобы выдавать **одну роль** — оставьте в массиве один ID:
    ```json
    "inviteRoles": ["1382738255249936554"]
    ```
  - Чтобы выдавать **три роли** — добавьте третий ID:
    ```json
    "inviteRoles": ["1382738255249936554", "1450198860419960935", "ID_ТРЕТЬЕЙ_РОЛИ"]
    ```
